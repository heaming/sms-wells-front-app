...
****************************************************************************************************
1. 모듈 : PSG
2. 프로그램 ID : WwpsgBusinessManagerReportMgtM - BM보고서 관리  (K-W-GM-U-0039M01)
3. 작성자 : jungsu.kim
4. 작성일 : 2023.05.16
****************************************************************************************************
* 프로그램 설명
...
async function getBusinessType() {
  const optionParams = { rpotBizTpDvCd: '01',
    ogCds: ['70632', '71321', '71301', '70533', '70529', '70532', '70078', '71776', '71774', '75050', '71948',
      '70653', '71314', '71361', '71356', '70202', '71309', '71346', '71807'] };
  const businessRes = await dataService.get('/sms/wells/competence/business-manager-report/business-type', { params: optionParams });
  // eslint-disable-next-line no-unused-vars
...
async function getBusinessTypeContent() {
  const optionParams = { rpotBizTpDvCd: '01', rpotBizTpId: saveParams.value.rpotBizTpId };
  const res = await dataService.get('/sms/wells/competence/business-manager-report/business-type-content', { params: optionParams });
...
async function fetchData() {
  const res = await dataService.get('/sms/wells/competence/business-manager-report/paging', { params: { ...cachedParams, ...pageInfo.value } });
...
async function onClickSave() {
  ...
  await dataService.post('/sms/wells/competence/business-manager-report/business-manager-report', saveRows);
...
async function onClickNtccnSave() {
  ...
  await dataService.post('/sms/wells/competence/business-manager-report/report-business-type', businessTypeParam);
  notify(t('MSG_ALT_SAVE_DATA'));
}
...
async function onClickRemove() {
  ...
  if (deletedRows.length > 0) {
    await dataService.delete('/sms/wells/competence/business-manager-report/', { data: deletedRows });
    await fetchData();
  }
}
  ...
  view.onCellButtonClicked = async (g, { itemIndex, fieldName }) => {
    if (fieldName === 'prtnrNo') {
      ...
      if (result) {
        ...
        const res = await dataService.get('/sms/wells/competence/business-manager-report/prtnr-info', { params: { ogTpCd: payload.ogTpCd, prtnrNo: payload.prtnrNo } });
