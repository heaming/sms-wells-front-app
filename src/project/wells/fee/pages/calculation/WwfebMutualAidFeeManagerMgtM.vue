...
****************************************************************************************************
1. 모듈 : FEB
2. 프로그램 ID : WwfebMutualAidFeeManagerMgtM - 상조 수수료 관리(M조직용)
3. 작성자 : 37612
4. 작성일 : 2023.12.07
****************************************************************************************************
* 프로그램 설명
...
async function fetchData() {
  ...
  const { data } = await dataService.get(`/sms/wells/fee/mutual-aid-manager/${fixApi}`, { params: { ...cachedParams } });
...
async function onClickCreate() {
  ...
  const { result: isChanged } = await modal({
    component: 'WwfebMutualAidFeeManagerRegP',
    componentProps: {
...
async function onClickReCreate() {
  ...
  const { result: isChanged } = await modal({
    component: 'WwfebMutualAidFeeManagerRedpRegP',
    componentProps: {
      baseYm: searchParams.value.baseYm,
    },
  });
