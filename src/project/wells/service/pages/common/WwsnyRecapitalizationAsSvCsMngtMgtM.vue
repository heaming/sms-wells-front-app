...
****************************************************************************************************
1. 모듈 : SNY
2. 프로그램 ID : WwsnyRecapitalizationAsSvCsMngtMgtM - 유상A/S 서비스비용 관리
3. 작성자 : kyunglyn.lee
4. 작성일 : 2023.03.08
****************************************************************************************************
* 프로그램 설명
...
const onChangeHgrPdCd = async () => {
  const res = await dataService.get('/sms/wells/service/recap-as-sv-cs-mngt/filter-products', { params: searchParams.value });
  pdGr.value = res.data;
};
...
async function fetchData() {
  const res = await dataService.get('/sms/wells/service/recap-as-sv-cs-mngt/paging', { params: { ...cachedParams, ...pageInfo.value } });
...
async function onClickExcelDownload() {
  const view = grdMainRef.value.getView();
  const res = await dataService.get('/sms/wells/service/recap-as-sv-cs-mngt/excel-download', { params: searchParams.value });
  await gridUtil.exportView(view, {
    fileName: 'RecapitalizationAsSvCsMngt',
    timePostfix: true,
    exportData: res.data,
  });
}
...
async function onClickSave() {
  ...
  if (chkRows.length === 0) {
    ...
    await dataService.post('sms/wells/service/recap-as-sv-cs-mngt', chkRows);
  ...
  const columns = [
    ...
    { fieldName: 'sumAmt',
      ...
      displayCallback(grid, index, value) {
        const csmrUprc = grid.getValue(index.itemIndex, 'csmrUprcAmt');
        const tcfee = grid.getValue(index.itemIndex, 'tcfeeAmt');
        if (csmrUprc !== null || tcfee !== null) {
          return parseInt(csmrUprc, 10) + parseInt(tcfee, 10);
        }
