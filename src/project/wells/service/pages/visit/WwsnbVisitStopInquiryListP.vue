 ...
 ****************************************************************************************************
 1. 모듈 : SNB
 2. 프로그램 ID : WwsnbVisitStopInquiryListP - 방문중지 조회
 3. 작성자 : seungsoo.kim
 4. 작성일 : 2023.03.23
 ****************************************************************************************************
 * 프로그램 설명
...
const initGrdMain = defineGrid((data, view) => {
  ...
  const columns = [
    ...
    { fieldName: 'apyDt',
      ...
      styleName: 'text-center',
      displayCallback(grid, index, value) {
        const apyDtReg = grid.getValue(index.itemIndex, 'apyDt');
        if (apyDtReg !== null) {
          return apyDtReg?.replace(/^(\d{4})(\d{2})(\d{2})$/, '$1-$2-$3');
        }

        return isEmpty(value) ? apyDtReg : value;
      },
    },
    {
      fieldName: 'sppStpDvNm',
      ...
      styleName: 'text-center',
      displayCallback(grid, index, value) {
        const sppStpDvCd = grid.getValue(index.itemIndex, 'sppStpDvCd');

        if (sppStpDvCd !== null) {
          return code.find((obj) => obj.codeId === grid.getValue(index.itemIndex, 'sppStpDvCd')).codeName;
        }

        return isEmpty(value) ? sppStpDvCd : value;
      },
    },
