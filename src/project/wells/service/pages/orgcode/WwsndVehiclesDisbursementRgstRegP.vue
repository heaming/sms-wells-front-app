 ...
 ****************************************************************************************************
 1. 모듈 : SND
 2. 프로그램 ID : WwsndVehiclesDisbursementRgstRegP - 업무차량 지급등록 팝업
 3. 작성자 : seungsoo.kim
 4. 작성일 : 2023.01.25
 ****************************************************************************************************
 * 프로그램 설명
...
<template>
  <kw-observer
    ref="vehicleDisbursementFormObsRef"
    name="vehicleDisbursementFormObs"
  >
    <kw-popup
      class="kw-popup--3xl"
    ...
    >
      ...
      <kw-form :cols="2">
        <kw-form-row>
          <!-- 서비스센터 -->
          <kw-form-item :label="$t('MSG_TXT_SV_CNR')">
            <kw-select
      ...
      <kw-form :cols="2">
        ...
        <kw-form-row>
          <!-- 차량번호 -->
          <kw-form-item :label="$t('MSG_TXT_CARNO')">
            <kw-select
...
import { codeUtil, notify, useDataService, useModal, getComponentType, useGlobal } from 'kw-lib';
...
const vehicleDisbursementFormObsRef = ref(getComponentType('KwObserver'));
...
async function fetchAllVehicles() {
  return await dataService.get('/sms/wells/service/business-vehicle/all-vehicles');
}
...
async function fetchBusinessVehicle(vhcMngtNo, vhcMngtSn) {
  return await dataService.get(`/sms/wells/service/business-vehicle/${vhcMngtNo}/${vhcMngtSn}`);
}
...
async function onClickSaveBtn() {
  if (!await vehicleDisbursementFormObsRef.value.validate()) { return; }
  if (await vehicleDisbursementFormObsRef.value.alertIfIsNotModified()) { return; }
  if (await validateDsbDate()) { return; }
  if (!await confirm(t('MSG_ALT_WANT_SAVE'))) { return; }

  // TODO: 엔지니어 테이블 작업 완료 후 WsndVehiclesDsbRgstService.java 주석 해제
  if (isModify.value) {
    await dataService.put('/sms/wells/service/business-vehicle', dataParams.value);
  } else {
    await dataService.post('/sms/wells/service/business-vehicle', dataParams.value);
  }
