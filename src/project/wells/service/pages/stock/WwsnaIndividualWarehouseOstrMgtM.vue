...
import { defineGrid, codeUtil, useDataService, getComponentType, useGlobal, gridUtil, useMeta } from 'kw-lib';
...
async function onClickLgstTrs() {
  ...
  // 물류 전송
  const res = await dataService.post('/sms/wells/service/individual-ware-ostrs/logistics-transfer', searchParams.value, { timeout: 300000 });
  const { processCount } = res.data;
  if (processCount > 0) {
    // 이관이 완료되었습니다.
...
const initGrdMain = defineGrid((data, view) => {
  const fields = [
    ...
    { fieldName: 'ostrPrtnrOgTpCd' }, // 출고창고 조직유형코드
  ];
