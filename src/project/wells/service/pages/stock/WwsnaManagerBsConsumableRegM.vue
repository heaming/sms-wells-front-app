 ...
 ****************************************************************************************************
 1. 모듈 : SNA
 2. 프로그램 ID : WwsnaManagerBsConsumableRegM(W-SV-U-0012M01) - 개인별 활동물품 배부현황 (매니저 활동물품)
 3. 작성자 : seungsoo.kim
 4. 작성일 : 2023.04.20
 ****************************************************************************************************
 * 프로그램 설명
...
import { defineGrid, useMeta, getComponentType, useDataService, gridUtil, notify, alert, confirm } from 'kw-lib';
...
async function getItems() {
  ...
  const res = await dataService.get(`/sms/wells/service/manager-bsconsumables/items/${mngtYm}`);
...
async function getMCsmbAplcClose() {
  ...
  const res = await dataService.get(`/sms/wells/service/manager-bsconsumables/time-limit/${mngtYm}`);
...
async function reAryGrid() {
  ...
  const fields = [
    ...
    { fieldName: 'vstCstN', dataType: 'number' }, // 방문계정수
  ...
  const columnLayout = [
    {
      ...
      direction: 'horizontal',
      items: ['reqYn', 'bldNm', 'bldCd', 'prtnrNo', 'ogCd', 'prtnrKnm', 'vstCstN'],
    },
    {
      header: `${t('MSG_TXT_BY_PRD')} ${t('MSG_TXT_VST_ACC')}`,
      direction: 'horizontal',
      items: ['wrfr', 'bdtIndv', 'bdtCrp', 'arcleIndv', 'arcleCrp', 'wtrSftnr', 'cffMchn', 'msgcr', 'dryr', 'wash', 'ardrssr', 'sscling'],
    },
    {
...
async function getApplicationLimitQty() {
  ...
  const res = await dataService.get(`/sms/wells/service/manager-bsconsumables/${mngtYm}/application-limit-qty`);
...
async function onClickRgstPtrmSe() {
  ...
  // 등록기간 저장
  const res = await dataService.post('/sms/wells/service/manager-bsconsumables/period-term', aplcCloseData.value);
...
async function onClickSave() {
  ...
  const res = await dataService.post('/sms/wells/service/manager-bsconsumables', saveData);
  const { processCount } = res.data;
  if (processCount > 0) {
    notify(t('MSG_ALT_SAVE_DATA'));
    await fetchData();
...
async function onClickOstrAk() {
  ...
  checkedRows.forEach((checkedRow) => {
    if (checkedRow.bfsvcCsmbDdlvStatCd !== '20') {
      // {0}의 신청 상태를 확인해 주세요.
      alert(`${checkedRow.prtnrKnm}(${checkedRow.prtnrNo})${t('MSG_ALT_APLC_STAT_CONF')}`);
      errorYn = true;
      return;
    }
...
const initGrdMain = defineGrid(async (data, view) => {
  const fields = [
    ...
    { fieldName: 'ogCd' }, // 조직코드
    { fieldName: 'prtnrKnm' }, // 성명
    { fieldName: 'vstCstN' }, // 방문계정수
    { fieldName: 'wrfr' }, // 정수기
    { fieldName: 'bdtIndv' }, // 비데(개인)
    { fieldName: 'bdtCrp' }, // 비데(법인)
    { fieldName: 'arcleIndv' }, // 공기청정기(개인)
    { fieldName: 'arcleCrp' }, // 공기청정기(법인)
    { fieldName: 'wtrSftnr' }, // 연수기
    { fieldName: 'cffMchn' }, // 커피머신
    { fieldName: 'msgcr' }, // 안마의자
    { fieldName: 'dryr' }, // 건조기
    { fieldName: 'wash' }, // 세탁기
    { fieldName: 'ardrssr' }, // 에어드레서
    { fieldName: 'sscling' }, // 삼성청소기
  ];
